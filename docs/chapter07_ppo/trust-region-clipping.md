...
$A_t < 0$ 的两种情形由对称性给出（数值皆负，"更小"指绝对值更大、惩罚更重）。四种越界情形汇总：
...
四种情形中，$\min$ 始终选取"更悲观"（数值更小）的那一项：顺方向越界时 clip 砍掉虚高的奖励，裁剪值更悲观；反方向越界时未裁剪值如实地暴露奖励确实很低，未裁剪值更悲观。**"更悲观"恰好等于"梯度方向正确"**——这是 $\min$ 能在所有越界情形中保证梯度非零（当且仅当需要纠偏时）的根本原因。
