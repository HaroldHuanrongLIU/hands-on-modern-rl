...
上一节我们推导出了贝尔曼方程，建立了状态价值函数 $V^\pi(s)$ 和它的递归结构：**当前价值 = 眼前奖励 + 折扣后的下一状态价值**。理论上，只要知道转移概率 $P$ 和奖励函数 $R$，就能用矩阵求逆算出所有状态的精确价值。但这恰恰是现实中最难满足的条件——CartPole 的物理公式确实能写出 $P$ 和 $R$，可围棋的状态多达 $10^{170}$ 种，大模型生成的 token 组合更是天文数字。在绝大多数实际问题中，**环境是一个黑盒**，我们拿不到完整的”世界说明书”。
...
- **MC（蒙特卡洛）**：抛开模型，蒙着眼跑完整局再看总分，回头修正每一步的判断 → 不用模型，但一局太长就得一直等。
