...
上一节我们建立了动作价值函数 $Q(s,a)$：它给每个 $(s,a)$ 对贴上分数牌，最优策略只需 $\arg\max_a Q^*(s,a)$——比较大小，选最高分。这条路在离散动作空间里非常干净，Q-Learning 和 DQN 都沿着它取得了巨大成功。但上一节末尾我们也看到了它的硬伤：**当动作空间变成连续的，$\arg\max$ 就失灵了。** 你没法给无穷多个动作逐一打分再挑出最大值。

考虑一个六轴机械臂：每个关节需要施加一个连续力矩，动作空间是 $\mathbb{R}^6$ 中的一个点，包含无穷多个可能的动作。再想想你跑过的 CartPole——如果把推力从”左/右”两个离散选项换成连续的推力大小，$Q$ 表格同样会爆炸。甚至在大语言模型的 RLHF 中，每一步要从整个词表中采样，动作空间虽然离散但极其巨大，维护一张完整的 $Q$ 表也不现实。

既然没法给每个动作打分，那就换一种思路：**不建打分表，直接学”看到什么局面就做什么动作”。** 具体来说，我们用一个参数化的策略 $\pi_\theta(a\mid s)$（比如一个神经网络）直接输出动作分布，然后用一个目标函数 $J(\theta)$ 衡量这套参数下的平均长期回报，最后通过梯度上升找到让 $J(\theta)$ 最大的参数 $\theta^*$。本节要回答的核心问题是：**如何把”找到一个好策略”写成可以优化的数学问题？**
