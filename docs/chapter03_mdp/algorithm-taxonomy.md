...
前两节我们走过两条优化路线：路线一学习 $Q(s,a)$，先给动作打分再选最高分；路线二优化 $J(\theta)$，直接调整策略让好动作更容易被选出来。3.6 节最终把它们整理成一张对比表——路线一打分准但只能处理离散动作，路线二天然支持连续动作空间但方差大。但无论是哪条路线，所有更新都要吃数据，而两节的讨论始终默认了一件事：**训练数据就是当前策略自己跑出来的。** 一旦把这个假设拿掉，一整排实际问题就会冒出来。

考虑你跑过的 CartPole：DQN 把几千条旧经验塞进 replay buffer 反复训练，这些经验很多是几十个 epoch 以前的旧策略采的——用旧策略的数据更新新策略，行不行？再看 PPO 训练大模型做 RLHF：每轮用当前模型生成一批回答，更新参数后模型已经变了，那这批回答还能接着用吗？用几次就得扔？再极端一点：DPO 根本不在线采样，只在一份人类标注好的固定偏好数据上训练——连环境交互都没有，这还算 RL 吗？这些场景的差异不在于损失函数长什么样，而在于一个更根本的问题：**训练用的那批样本，到底是谁产生的？产生之后还能不能继续采新的？**

回答这两个问题需要两条独立的概念轴。On-policy / Off-policy 问的是：**这批样本是不是由”我要更新的策略”自己采出来的？** Online / Offline 问的是：**训练过程中，我还能不能继续让策略去采新样本？** 这两个问题很像，但不是一回事——DQN 可以一边继续玩游戏、一边复用旧经验，所以它是 online + off-policy；PPO/GRPO 每轮让当前模型生成新回答再更新，所以常被看作 online + on-policy；DPO 在一份固定偏好数据上训练，所以更接近 offline 偏好优化。本节要做的，就是用这两条轴把 REINFORCE、Sarsa、Q-Learning、DQN、PPO、GRPO、CQL、DPO 等算法摆到正确的位置上。名字只是结果，数据关系才是原因。
