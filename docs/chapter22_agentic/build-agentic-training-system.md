...
这种隔离环境需要满足三个条件：接收 Agent 的动作（代码）、安全执行并限制资源、返回执行结果和终止状态。这就是 **Environment** 组件的职责，也是 22.2 节讨论的沙箱问题的最小实现。
...
- `step()` 接受结构化的 action（`action_type` + `action_args`），不是纯文本。这对应 22.1 节的动作空间 $A = A_{\text{text}} \cup A_{\text{action}}$
