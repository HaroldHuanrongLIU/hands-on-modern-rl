...
- 掌握 Q-Network 的架构与损失函数：用神经网络近似 $Q(s,a)$，通过均方 TD Error 训练。
...
> **DQN 损失函数（MSE TD Error）：**
>
> - $\theta$：Q-Network 的参数——正在被训练的"学生"。
> - $\theta^-$：目标网络的参数——一个慢更新的副本，专门用来生成"标准答案"。
> - $Q(s, a; \theta)$：Q-Network 对当前状态-动作对的打分，即"学生的回答"。
> - $r + \gamma \max_{a'} Q(s', a'; \theta^-)$：TD Target，即时奖励加上目标网络给出的打折未来最高分，即"标准答案"。
> - 外层 $\mathbb{E}$：对所有可能的转移 $(s, a, r, s')$ 取期望，实践中从经验回放池随机采样来近似。

**为什么需要这些公式**

上一节我们看到了 Q-Learning 的表格在高维状态空间中装不下，而直接用神经网络替代表格又会导致训练崩溃。崩溃的原因有两个：相邻帧高度相关让梯度方向被最近经历主导，以及参数共享让更新目标跟着一起动。DQN 的损失函数把这些问题的解法统一在一个框架里——经验回放池保证采样独立，目标网络让标准答案在一段时间内固定不动，均方误差驱动 Q-Network 逐步逼近稳定的 TD Target。三个组件各司其职，共同把"用神经网络学 Q 值"这件本来会崩溃的事变成了一件可行的事。

提醒一下：第 3 章我们学过 **TD 方法**的核心思想——走一步就能更新价值估计，不需要等一整局结束。TD Target = $r + \gamma V(s')$，即"已经落袋的奖励 + 下一状态的估计价值"。DQN 里的 TD Target 结构完全一样，只不过把 $V(s')$ 换成了 $\max_{a'} Q(s', a')$——因为 Q-Learning 评估的是动作价值，不是状态价值。
...
经验回放解决了"数据从哪来"和"样本相关性"的问题。但还有一个问题没有解决：损失函数里的 TD Target $r + \gamma \max_{a'} Q(s', a'; \theta^-)$ 本身是不稳定的。如果 TD Target 也在跟着网络一起变，网络就会像追自己尾巴的狗一样永远追不上。这就引出了第三个、也是最后一个组件。
